...
global Aero Rotor Profiles Angles Controller Nacelle Algo Shaft Environment Tower
nB=Rotor.nB;
ne=Rotor.ne;
r=Rotor.r;
R=Rotor.R;
...
% Forces and Power
BladeTorque=zeros(1,nB);
BladeThurst=zeros(1,nB);
...
for idB=1:nB
    ...
    for e=1:ne
        ...
        Vrel_in4=V0_in4+Aero.last.W(:,idB,e)+Vb_in4 + Velasicity_in4;    %relative speed
    ...
    BladeTorque(idB)=getTorqueFromBlade(r,Pt,Rotor.R);
    BladeThrust(idB)=getThrustFromBlade(r,Pn,Rotor.R);
    BEM.BladeFlap(idB)=trapz([Rotor.r;R],[Rotor.r.*(Pn);0]);
end %loop on blades


%%%% Returning Aerodynamic Forces
BEM.Torque = sum(BladeTorque);   %Rotor shaft torque at t in N
BEM.Thrust = sum(BladeThrust);   %Rotor shaft thrust at t in N
