...
for idT=1:length(Vtime)
    ...
    Aero.Wind.V0=VV0_of_t(mod(idT,length(VV0_of_t(:,1)))+1,:);
    ...
    for idB=1:nB
        ...
        for e=1:Rotor.ne
            ...
            % Incoming wind
            V0_in1=getPointIncomingWind(r_position(e,:), Angles.psi);
            V0_in4=a41'*V0_in1';
            V0_in3=a31'*V0_in1';
            ...
            if(isequal(Algo.Format,'HAWC'))  
                 ClCdCm= fAeroCoeff(alpha,Profiles,Rotor.ProfileSet(e,:),Rotor.thickness_rel(e));
                 BEM.Cl=ClCdCm(:,1);
                 BEM.Cd=ClCdCm(:,2);
            else %flex
            ...
            BEM.L=0.5*Environment.rho*norm(Vrel_in4).^2*Rotor.chord(e)*BEM.Cl;
            BEM.D=0.5*Environment.rho*norm(Vrel_in4).^2*Rotor.chord(e)*BEM.Cd;
            ...
            W_z_qs=-nB*BEM.L*cosd(phi)/(4*pi*Environment.rho*r(e)*BEM.F*norm(V0_in4+fg*nnW_in4 ));
            W_y_qs=-nB*BEM.L*sind(phi)/(4*pi*Environment.rho*r(e)*BEM.F*norm(V0_in4+fg*nnW_in4 ));
            ...
            if(e==Rotor.e_ref_for_khi)
                ...
                V0_in1=getPointIncomingWind(r_hub, Angles.psi);
                V0_in2=a12*V0_in1';
                % psi0
        ...
        BladeTorque(idT,idB)=getTorqueFromBlade(r,Pt,Rotor.R);
        BladeThrust(idT,idB)=getThrustFromBlade(r,Pn,Rotor.R);        
