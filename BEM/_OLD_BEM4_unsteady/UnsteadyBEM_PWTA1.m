...
if(BigStorage)
    Mpsi=zeros(length(Vtime),nB);
    MW=zeros(length(Vtime),3,nB,ne);
    MW0=zeros(length(Vtime),3,nB,ne);
    MPn=zeros(length(Vtime),nB,ne);
    MKhi=zeros(length(Vtime),nB);
end
...
for idT=1:length(Vtime)
    ...
    [a12 a23 a34]=getTransfoMatrices(Angles);
    ...
    for idB=1:nB
        ...
        for e=Velements
            ...
            % Incoming wind
            V0_in1=getPointIncomingWind(r_position(e,:), Tower, Angles, Model,VelocityParams);
            V0_in4=a41'*V0_in1';
            V0_in3=a31'*V0_in1';
            ...
            if(e==e_ref_for_khi)
                ...
                V0_in1=getPointIncomingWind(r_hub, Tower, Angles, Model,VelocityParams);
                V0_in2=a12*V0_in1';
                % psi0
        ...
        Torque(idT,idB)=getTorqueFromBlade(r,Pt);
    ...
    M_tot(idT) = sum(Torque(idT,:))/1000;   %Rotor shaft torque for the given time in kN
end % time loop
P=omega*M_tot;
