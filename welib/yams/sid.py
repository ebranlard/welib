...
# ---FAST 2 SID
# --------------------------------------------------------------------------------{
def FASTBlade2SID(ed_file=None, p=None, Imodes_bld=[0,1,2]):
    """ 
    Create a SID from either:
      - an OpenFAST ElastoDyn file
     OR
      - a set of parameteters generated by bladeParameters
    """
    ...
    p = bladeDerivedParameters(p, inertiaAtBladeRoot=True) # TODO

    nq = len(Imodes_bld)
    sid=SID(nq=nq)

    sid.refmod.mass= p['BldMass']
    for i in np.arange(nq):
        sid.refmod.ielastq[i]= 'Eigen Mode {:4d}'.format(i)
    # --- mdCM
    sid.mdCM.M0= p['mdCM']
    ...
    #sid.ModalShapes{1}= squeeze(p.TwistedSF(1, 1:2, 1, :, 1))';
    #sid.ModalShapes{2}= squeeze(p.TwistedSF(1, 1:2, 3, :, 1))';
    #sid.R= [0 p.RNodes p.BldFlexL]'+p.HubRad;
...
def FASTTower2SID(ed_file=None, p=None, gravity=None, RotMass=None, Imodes_twr=[0,1,2,3]):
...
#     if twr:
    ...
    # --- OpenFAST Method
    if method=='OpenFAST':
        # We always need the blade parameters
    ...
    if Imodes_twr is not None:
        if method=='FEM':
            ...
            twr_sid = FASTTower2SID(ed_file, Imodes_twr=Imodes_twr, gravity=gravity, RotMass=RotMass)
        else:
    ...
    if Imodes_bld is not None:
        if method=='FEM':
            ...
            bld_sid = FASTBlade2SID(p=pbld[0], Imodes_bld=Imodes_bld)
        else:
            raise NotImplementedError()
