...
#   0.000000E+00  0.000000E+00  9.337225E+01
    ...
    def simulate_py(self, model, p, time=None, u=None, refOut=None):
    ...
    def simulate_py_lin(self, model, p, time=None, uop=None, refOut=None, qop=None, qdop=None, du=None):
        ...
        # --- Reference simulation
        if refOut is not None:
            df=weio.read(fstFilename.replace('.fst','.out')).toDataFrame() # TODO remap
            #time = np.linspace(0,50,5000)
            time = df['Time_[s]'].values
        elif time is None:
            raise Exception('Time vector must be provided if no reference simulation is given')
        ...
        if uop is None and refOut is None:
