...
class TNSBStructure(WindTurbineStructure):
    ...
    def __init__(self, 
                 ...
                 theta_tilt=0, theta_yaw=0, theta_cone=0, 
                 ):
...
#         Fx_inE = T*cos(s.theta_tilt)
    ...
    def __repr__(self):
        ...
        s+=f' * Origin R  : {self.bld[0].r_O.T}\n'
        s+=f' * Origin S  : {self.hubgen.r_O.T}\n'
        s+='----------------- RNA ---------------------------------------\n'
    ...
    def auto_assembly(self, q=None, DEBUG=False, fixedShaft=False):
        ...
        bld = self.bld
        twr = self.twr
        theta_tilt_y =  self.shaft_tilt
        theta_cone_y =  self.theta_cone_y
        theta_yaw    =  self.theta_yaw
        ...
        if self.main_axis=='x':
            ...
            if self.bTiltBeforeNac:
                R_cn0 = np.dot(R_x (self.theta_yaw) , R_y (theta_tilt_y))
                R_cs0 = R_z (np.pi)
            else:
                R_cn0 = R_x (self.theta_yaw) 
                R_cs0 = np.dot( R_y(theta_tilt_y) , R_z (np.pi)) # Note: OrientBefore
            Shaft_axis='z'
        elif self.main_axis=='z':
            if self.bTiltBeforeNac:
                R_cn0 = np.dot(R_z (self.theta_yaw) , R_y(theta_tilt_y))
                R_cs0 = R_x (np.pi)
            else:
                R_cn0 = R_z (self.theta_yaw) 
