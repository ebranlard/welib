...
class HydroDyn:
    ...
    def init(self, gravity = 9.81, WtrDens='1025', WtrDpth=0, MSL2SWL=0):
        ...
        f = self.File
        # TODO handle default
        self.p['WtrDpth'] = f['WtrDpth'] + f['MSL2SWL'] 
        graph = self.graph
        # --- Morison
        # see Morison_GenerateSimulationNodes in Morison.f90
        ...
        Waves_WaveKin_Nodes = self.MorisonPositions
        Current_NodesZ = self.MorisonPositions[:,2]
        ...
        # Swap nodes if needed, see SetupMembers in Morison.f90
        for m in Mgraph.Elements:
            print(m.nodeIDs)
...
#                call SetNodeMG( InitInp%NMGDepths, InitInp%MGDepths, InitInp%Nodes(member%NodeIndx(i)), InitInp%MSL2SWL, member%tMG(i), member%MGDensity(i) )
            ...
            C = Mgraph.NodePropertySets['SimpleCoefs'][0] # we only have one of these sets
            print(C.keys())
            ...
            # TODO Filled members
            if 'FillGroups' in Mgraph.NodePropertySets.keys():
                # Find index 
                raise NotImplementedError()
            ID = -1
            if ID>0:
...
#                     member%CM0_fb(i) = TwoPi *                     member%FillDens * gravity * dl *( 0.25*dl**3* member%dRdl_in(i)**4 + 0.25*dl**3* member%dRdl_in(i)**2 + dl**2* member%dRdl_in(i)**3*member%Rin(i) + 2.0/3.0*dl**2* member%dRdl_in(i)*member%Rin(i) + 1.5*dl* member%dRdl_in(i)**2*member%Rin(i)**2 + 0.5*dl*member%Rin(i)**2 +  member%dRdl_in(i)*member%Rin(i)**3 )
    ...
    #       call SetNodeMG( InitInp%NMGDepths, InitInp%MGDepths, InitInp%Nodes(i), InitInp%MSL2SWL, InitInp%Nodes(i)%tMG, InitInp%Nodes(i)%MGDensity )
        NJoints = len(Mgraph.Nodes)
        NNodes  = len(self.MorisonPositions) # TODO
        MMisc  = dict()
        MParam = dict()
        MMisc['nodeInWater']   = np.zeros(NNodes)
        MMisc['vrel']          = np.zeros((    3, NNodes   ))
        MMisc['FV']            = np.zeros((    3, NNodes   ))
        MMisc['FA']            = np.zeros((    3, NNodes   ))
        MMisc['FDynP']         = np.zeros(       NNodes     )
        MMisc['F_I_End']       = np.zeros((    3, NJoints))
        MMisc['F_BF_End']      = np.zeros((    6, NJoints))
        MMisc['F_A_End']       = np.zeros((    3, NJoints))
        MMisc['F_D_End']       = np.zeros((    3, NJoints))
        MMisc['F_B_End']       = np.zeros((    6, NJoints))
        MMisc['F_IMG_End']     = np.zeros((    6, NJoints))
        MParam['An_End']       = np.zeros((    3, NJoints))
        MParam['DragConst_End']= np.zeros(        NJoints  )
        MParam['I_MG_End']     = np.zeros(( 3, 3, NJoints))
        MParam['F_WMG_End']    = np.zeros((    3, NJoints))
        MParam['Mass_MG_End']  = np.zeros(        NJoints )
        MParam['AM_End']       = np.zeros(( 3, 3, NJoints))
        MParam['DP_Const_End'] = np.zeros((    3, NJoints))
        MParam['nodeInWater']  = nodeInWater  # At each time step
#    
        # ---- Input Mesth
        # u%Mesh
...
def hydrodynToSum(hd, filename, more=False):
    """ 
    Write a summary file, similar to HydroDyn
    """
    Mgraph = hd.Mgraph
    MSL2SWL = hd.File['MSL2SWL']
...
#        CALL MeshMapDestroy( M_P_2_P, errStat, errMsg ); IF ( errStat /= ErrID_None ) CALL WrScr(TRIM(errMsg))
        ...
        # Write the node data
        for i,n in enumerate(Mgraph.Nodes):
            ...
            s+=s0
        c = len(Mgraph.Nodes)
        for j,e in enumerate(Mgraph.Elements):
            m = e.MorisonData
            print(e)
            for i in np.arange(1,m['NElements']):
                ...
                # need to add MSL2SWL offset from this because the Positons are relative to SWL, but we should report them relative to MSL here
                pos = m['SubNodesPositions'][i-1, :]
                pos[2] += MSL2SWL
                s0= ' {:5d}  {:10d}  {:10.4f}  {:10.4f}  {:10.4f}  {:10.3e}  {:10.3e}  {:10.3e}  {:10.3e}'.format(c,e.ID, pos[0], pos[1], pos[2], m['R'][i], m['R'][i]-m['Rin'][i], m['tMG'][i], m['MGdensity'][i])
                spropot='T' if e.data['Pot'] else 'F'
                sfill  ='T' if fillFlag      else 'F'
                s0+= '           {:s}           {:s}'.format(spropot, sfill)
                s0+= '  {:10.3e}  {:10.3e}  {:10.3e}  {:10.3e}  {:10.3e}  {:10.3e}  {:10.3e}'.format(m['m_fb_u'][i]+m['m_fb_l'][i],  m['Cd'][i], m['Ca'][i], m['Cp'][i], m['AxCd'][i], m['AxCa'][i], m['AxCp'][i])
                s0+= '         -           -           -  \n'
                s0 = s0.replace('e+','E+').replace('e-','E-')
                s+=s0
        s+='\n'
        ...
        for i, e in enumerate(Mgraph.Elements):
            d = e.data
            m = e.MorisonData
            n1 = e.nodes[0]
            n2 = e.nodes[1]
