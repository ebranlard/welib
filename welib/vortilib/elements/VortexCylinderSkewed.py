...
def svc_tang_u_polar(vr,vpsi,vz,gamma_t=-1,R=1,m=0,ntheta=180,polar_out=False):
    ...
    if polar_out:
        ...
        for i,(r,psi,z) in enumerate(zip(vr,vpsi,vz)):
            ...
            # Integrations
            u_r[i]    = np.trapezoid((ar * np.sqrt(c)+ np.multiply(br,np.sqrt(a)))*D, vtheta)
            u_psi[i]  = np.trapezoid((ap * np.sqrt(c)+ np.multiply(bp,np.sqrt(a)))*D, vtheta)
            u_z[i]    = np.trapezoid((az * np.sqrt(c)+ np.multiply(bz,np.sqrt(a)))*D, vtheta)
...
#             # Integrations
#             u_x[i]    = np.trapezoid((b*bx-2*ax*c)*D, vtheta)
#             u_y[i]    = np.trapezoid((b*by-2*ay*c)*D, vtheta)
#             u_z[i]    = np.trapezoid((b*bz-2*az*c)*D, vtheta)
        ...
        for i,(r,psi,z) in enumerate(zip(vr,vpsi,vz)):
            ...
            D = 2*gamma_t/(4*np.pi)/(np.multiply(np.sqrt(a),(2 * np.sqrt(a * c)+ b)))
            # Integrations
            u_x[i]    = np.trapezoid((ax * np.sqrt(c)+ np.multiply(bx,np.sqrt(a)))*D, vtheta)
            u_y[i]    = np.trapezoid((ay * np.sqrt(c)+ np.multiply(by,np.sqrt(a)))*D, vtheta)
            u_z[i]    = np.trapezoid((az * np.sqrt(c)+ np.multiply(bz,np.sqrt(a)))*D, vtheta)
...
def svc_longi_u_polar(vr,vpsi,vz,gamma_l=-1,R=1,m=0,ntheta=180,polar_out=False):
    ...
    if polar_out:
        ...
        for i,(r,psi,z) in enumerate(zip(vr,vpsi,vz)):
            ...
            u_r[i]   = np.trapezoid((  - m*z*np.sin(psi) + np.sin(vtheta-psi))*DenInv,vtheta)
            u_psi[i] = np.trapezoid((r - m*z*np.cos(psi) - np.cos(vtheta-psi))*DenInv,vtheta)
            u_z[i]   = np.trapezoid(m * (-np.sin(vtheta) + r*np.sin(psi))     *DenInv,vtheta)
        ...
        for i,(r,psi,z) in enumerate(zip(vr,vpsi,vz)):
            ...
            Den2 = - z + m * np.cos(vtheta) + np.sqrt(1 + m ** 2) * np.sqrt(1 + r ** 2 + z ** 2 - 2 * r * np.cos(vtheta - psi)) - m * r * np.cos(psi)
            DenInv = gamma_l/(4*np.pi)/np.multiply(Den1,Den2)
            u_x[i]   = np.trapezoid(        (np.sin(vtheta) - r*np.sin(psi))  *DenInv,vtheta)
            u_y[i]   = np.trapezoid((- m*z - np.cos(vtheta) + r*np.cos(psi))  *DenInv,vtheta)
            u_z[i]   = np.trapezoid(m * (-np.sin(vtheta) + r*np.sin(psi))     *DenInv,vtheta)
        # Reshaping to input shape
        u_x   =  u_x.reshape(shape_in)   
        u_y   =  u_y.reshape(shape_in)   
...
def fKxit(vr,m):
    ...
    for ir in np.arange(len(vr)):
        r = vr[ir]
        Kxit_num[ir] = 2 * r / np.pi * np.trapezoid(np.sin(2 * vtheta) ** 2.0 / (np.multiply(np.sqrt((1 + r) ** 2 - 4 * r * np.cos(vtheta) ** 2),((r - np.cos(2 * vtheta)) ** 2 * np.cos(chi) ** 2 + np.sin(2 * vtheta) ** 2))), vtheta)
...
def fKzt(r,m,nout=2):
    ...
    for ir,r in enumerate(vr):
        Kztnum[ir] = - 1 / (np.pi) * r * np.sqrt(1 + m ** 2) / m * np.trapezoid(- 1.0 / (np.sqrt(1 + r ** 2 - 2 * r * np.cos(vtheta))) + np.sqrt(1 - 2 * r * np.cos(vtheta) + r ** 2) / (1 + r ** 2 - 2 * r * np.cos(vtheta) + m ** 2 * np.sin(vtheta) ** 2),vtheta)
    ...
    if nout==1:
        ...
        vtheta = np.linspace(0,np.pi,1000)
        for ir,r in enumerate(vr):
            Kztnum2[ir] = r * m * np.sqrt(1 + m ** 2) / np.pi * np.trapezoid(np.sin(vtheta) ** 2.0 / (np.multiply(np.sqrt(1 + r ** 2 - 2 * r * np.cos(vtheta)),(1 + r ** 2 - 2 * r * np.cos(vtheta) + m ** 2 * np.sin(vtheta) ** 2))),vtheta)
        # My formula Alternative form3 (WEIRD RESULTS !!!!!!!!!!!!)
        vtheta = np.linspace(0,np.pi / 2,1000)
        for ir,r in enumerate(vr):
            Kztnum3[ir] = 2 * r * np.sqrt(1 + m ** 2) * m / np.pi * np.trapezoid(np.sin(2 * vtheta) ** 2.0 / (np.multiply(np.sqrt((1 + r) ** 2 - 4 * r * np.cos(vtheta) ** 2),((1 + r) ** 2 - 4 * r * np.cos(vtheta) ** 2 + m ** 2 * np.sin(2 * vtheta) ** 2))),vtheta)
        # My formula Alternative form4
        vtheta = np.linspace(0,np.pi / 2,1000)
        for ir,r in enumerate(vr):
            Kztnum4[ir] = 2 * r / np.pi * 1 * np.sin(chi) * np.trapezoid(np.sin(2 * vtheta) ** 2.0 / (np.multiply(np.sqrt((1 + r) ** 2 - 4 * r * np.cos(vtheta) ** 2),((r - np.cos(2 * vtheta)) ** 2 * np.cos(chi) ** 2 + np.sin(2 * vtheta) ** 2))),vtheta)
